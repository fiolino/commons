...
public class Methods {
  ...
  private static <V> V iterateOver(Lookup lookup, Class<?> type, V initialValue, MethodVisitor<V> visitor,
    ...
    do {
      ...
      outer : for (final Method m : methods) {
  ...
  public static MethodHandle returnArgument(MethodHandle target, int argumentNumber) {
    ...
    MethodType targetType = target.type();
    if (argumentNumber >= targetType.parameterCount()) {
      throw new IllegalArgumentException("Handle " + target + " has too few arguments: " + argumentNumber);
    }
  ...
  private static <T> T doInClassHierarchy(Class<?> type, Function<? super Class<?>, T> action) {
    Class<?> c = type;
  ...
  private static String generateNameFrom(MethodHandleInfo info) {
    return info.getDeclaringClass().getSimpleName() + "::" + info.getName() + info.getMethodType();
  }
//
//  public static MethodHandle lambdafy(Lookup lookup, Class<I> lambdaType,
//                               MethodHandle targetMethod) {
//
//    Method m = findLambdaMethod(lambdaType);
//    MethodHandleInfo info;
//    try {
//      info = lookup.revealDirect(targetMethod);
//    } catch (IllegalArgumentException ex) {
//      // Not a direct handle
//      return null;
//    }
//    String name = generateNameFrom(info);
//    try {
